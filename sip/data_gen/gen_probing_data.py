...
if __name__ == "__main__":
    t0 = time.time()
    # random.seed(55)
    random.seed(23434653) # used for further pre-training data for experiments on bimachines.
    ...
    #name = f"pretrain_s{DESIRED_MAX_STATES}_more"
    ...
    with (open(f"data/pretrain/probing_train_{name}.jsonl", "w") as f_train,
          pynini.Far(f"data/pretrain/probing_train_{name}.far", mode="w") as far_train,
          pynini.Far(f"data/pretrain/probing_dev_{name}.far", mode="w") as far_dev,
          pynini.Far(f"data/pretrain/probing_test_{name}.far", mode="w") as far_test,
          open(f"data/pretrain/probing_dev_{name}.jsonl", "w") as f_dev,
          open(f"data/pretrain/probing_easy_dev_{name}.jsonl", "w") as easy_dev_f,
          open(f"data/pretrain/probing_test_{name}.jsonl", "w") as f_test):
        ...
        for fst, chosen_vocab in tqdm.tqdm(fst_collection):
            ...
            data_points = []
            faster_fst_access = FasterFSTAccess(fst_for_sampling)
            for _ in range(num_ex_per_task):
                ...
                data_points.append({"FST": fst_as_json, "input": inp, "output": o, "task_id": task_id,
                                    "state_seq": identify_state_sequence(faster_fst_access, inp)})
